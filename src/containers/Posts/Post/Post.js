    ...
    return (
        ...
        <div className={cs(classnames.post_item)}>
            <div className={cs(classnames.post_item_author_infor)}>
                <Avatar style={{marginRight : '10px'}} alt="Remy Sharp" src={data.user.imagePath || null} />
                <div>
                    <h5 className={cs(classnames.authorName)}>{data.user.nickName} {data.user.lastName}</h5>
                    <p className={cs(classnames.postTime)}> {postMoment.format('LL')} {`(${postMoment.fromNow()})`}</p>
                </div>
            ...
            <div className={cs(classnames.post_item_tag)}>
                {data.tags ? data.tags.map(el => <Chip onClick={() => history.push(`/tag/${el.id}`)} style={{marginRight : "5px",cursor : 'pointer'}} key={el.id} label={el.name}/> ) :'No tags'}
            </div>
            ...
            <div className={cs(classnames.post_item_util)}>
                <div className={cs(classnames.post_item_reactions)}>
                    ...
                    <Button style={{textTransform: 'initial'}}>
                        {data.numberOfComments ? `${data.numberOfComments} comments` : 'Add comment'}
                    </Button>
                </div>
